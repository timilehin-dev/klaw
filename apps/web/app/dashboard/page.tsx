...
export default function DashboardPage() {
  ...
  // Bootstrap: create a thread only if none in URL
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (res.ok && json.id) {
          ...
          router.replace(`/dashboard?thread=${json.id}`);
        }
      } catch (e: any) {
        setError(e.message || "Could not create thread");
      }
    })();
  }, [threadFromUrl, router]);
  ...
  const fetchThreadData = useCallback(async (threadId: string) => {
    // Server API uses service role (works with workspace-scoped RLS)
        ...
        {messages.length === 0 && logs.length === 0 && (
          <div className="h-full min-h-[200px] flex flex-col items-center justify-center text-center text-text-muted text-sm gap-2">
            ...
            <p className="text-xs max-w-sm">
              Resume prior threads from the left sidebar. Free MCP tools:
              time, sequential-thinking, and more.
            </p>
          </div>
        )}
      ...
      <div className="p-4 border-t border-border bg-surface shrink-0">
        <div className="flex items-center gap-2 border border-border rounded-lg p-2 bg-background">
          ...
          <button
            ...
            className="bg-primary text-white text-xs px-3 py-1 rounded-md hover:bg-primary-hover disabled:opacity-50"
          >
            {sending ? "…" : "Send"}
          </button>
        </div>
